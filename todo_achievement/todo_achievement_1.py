    ...
    def list_ideal(self):
        ...
        if hour == 17 :
            if (0<=minute) and (minute<=30):
                prob = prob = ((hour-10) * 6 + minute) / 450
            else :
                prob = 1
...
#print("理想消化率: ", todo_ideal,"%", sep='')
    ...
    def make_json(self):
        d = {
            'date':self.date,
            'Your Achievement Rate':self.YourAchieve,
            'Ideal Achievement Rate':self.IdealAchieve,
            'Level of Achievement':self.LevelAchieve,
            'True(1) or False(0)':self.TF
        }
